    ...
    def detect_char(self, visual=True):
        ...
        if visual:
            for rect in rects:
                ...
                cv2.rectangle(
                    self.vis, rect[0:2], (rect[0] + rect[2], rect[1] + rect[3]), (0, 0, 255), 2)
        else:
            for rect in rects:
                self.rects['ch'].append(rect)
    ...
    def detect_text(self, visual=True):
        ...
        for channel in channels:
            ...
            er1 = cv2.text.createERFilterNM1(
                erc1, 16, 0.00015, 0.13, 0.2, True, 0.1)

            erc2 = cv2.text.loadClassifierNM2('text/trained_classifierNM2.xml')
            er2 = cv2.text.createERFilterNM2(erc2, 0.5)

            regions = cv2.text.detectRegions(channel, er1, er2)
            rects = cv2.text.erGrouping(
                ...
                for r in range(0, np.shape(rects)[0]):
                    ...
                    cv2.rectangle(
                        vis, (rect[0], rect[1]), (rect[0] + rect[2],
                                                  rect[1] + rect[3]), (0, 255, 255), 2)

            else:
                for r in range(0, np.shape(rects)[0]):
                    self.rects['text'].append(rects[r])
    ...
    def getTextBlocks(self):
        ...
        for k in self.rects:
            for r in self.rects[k]:
                x1_, y1_, w, h = r
                x1, y1, x2, y2 = min(x1, x1_), min(
                    y1, y1_), max(x2, x1_ + w), max(y2, y1_ + h)
        # cv2.rectangle(self.vis, (x1,y1), (x2,y2), (0, 255, 255), 2)
        # self.visualize()
        cropped_img = self.vis[y1:y2, x1:x2]
        # cv2.imshow('cropped',cropped_img)
        cv2.imwrite(
            self.image_path.replace('.jpg', '_cropped.jpg'), cropped_img)
        return x1, y1, x2, y2


if __name__ == '__main__':
    detector = Detector('data/scenetext_segmented_word01.jpg')
