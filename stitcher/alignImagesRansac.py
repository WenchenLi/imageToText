...
class AlignImagesRansac(object):
    ...
    def __init__(self, image_dir, key_frame, output_dir, img_filter=None):
        ...
        try:
            ...
            if img_filter:
                ...
                self.dir_list = filter(
                    lambda x: x.find(img_filter) > -1, self.dir_list)
        ...
        self.dir_list = map(
            lambda x: os.path.join(image_dir, x), self.dir_list)
    ...
    def findDimensions(self, image, homography):
        ...
        for pt in [base_p1, base_p2, base_p3, base_p4]:
            ...
            normal_pt = np.array(
                [hp_arr[0] / hp_arr[2], hp_arr[1] / hp_arr[2]], np.float32)
    ...
    # {      /* ARGB or RGBA image */
        ...
        base_img = cv2.GaussianBlur(
            cv2.cvtColor(base_img_rgb, cv2.COLOR_BGR2GRAY), (5, 5), 0)
        ...
        for next_img_path in self.dir_list:
            ...
            next_features, next_descs = detector.detectAndCompute(
                next_img, None)

            matches = matcher.knnMatch(
                next_descs, trainDescriptors=base_descs, k=2)
        ...
        self.dir_list = filter(
            lambda x: x != closestImage['path'], self.dir_list)
        ...
        if (closestImage['inliers'] > 0.1):  # and
            ...
            (min_x, min_y, max_x, max_y) = self.findDimensions(
                closestImage['img'], H_inv)
            ...
            base_img_warp = cv2.warpPerspective(
                base_img_rgb, move_h, (img_w, img_h))
            print "Warped base image"

            # utils.showImage(base_img_warp, scale=(0.2, 0.2), timeout=5000)
            # cv2.destroyAllWindows()

            next_img_warp = cv2.warpPerspective(
                closestImage['rgb'], mod_inv_h, (img_w, img_h))
...
if __name__ == '__main__':
    ...
    #         name='base'
    #     else: name = str(curN)
    #     cv2.imwrite("stitchtest/"+name + ".jpg", frame)
    #     if cv2.waitKey(1) & 0xFF == ord('q'):
    #         break
    AlignImagesRansac('stitchtest/', 'stitchtest/base.jpg', 'stitchtest/')
