...
def swt(name, searchDirection):
    ...
    cv2.imwrite("data/canny.jpg", edgeMap)

    vis = cv2.cvtColor(vis, cv2.COLOR_BGR2GRAY)
    final = vis & edgeMap
    cv2.imwrite("data/fin.jpg", final)
    ...
    cv2.imwrite("data/gradx.jpg", dx)
    cv2.imwrite("data/grady.jpg", dy)
    ...
    cv2.imwrite("data/swt_pass2.jpg", swtMap)
    ...
    plt.show()
    cv2.imwrite("data/dx.jpg", dx)
    print 'sizeOfStrokePoints:', sizeOfStrokePoints
